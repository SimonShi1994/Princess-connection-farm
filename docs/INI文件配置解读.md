...
|selected_emulator|使用的模拟器的名称|string|若使用蓝蝶5国际版Nougat x64，且运行在 Hyper-V 环境下，请填写为 `蓝蝶5HyperV`，并检查配置 `bluestacks5_hyperv_conf_path`|雷电|
|bluestacks5_hyperv_conf_path|蓝蝶5国际版配置文件路径|string|当 `selected_emulator` 配置为 `蓝蝶5HyperV` 时，读取此配置指向的配置文件以获取模拟器端口|C:/ProgramData/BlueStacks_nxt/bluestacks.conf|
